...
export function exportUnlocksData() {
  ...
  const unlocksData: Unlock[] = [
    ...
    },
    {
      name: 'Cascade',
      conditions: computed(() => {
        const HasPuitEau = buildingsStore.wizardBuildings.some(
          (building) => building.id === 'puitDeMana',
        )
    ...
    },
    {
      ...
      conditions: computed(() => {
        return (
          wizardStore.ressources.incremental.coins >= 100 &&
          buildingsStore.wizardBuildings.some((building) => building.id === 'entrepot')
        )
      }),
    },
    {
